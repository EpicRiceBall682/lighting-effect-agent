...
- `src/structured_gradient.py`：从提示词构建 1～4 节点横向主渐变，并仅保留受限的 LoRA 低频亮度残差
- `src/quality.py`：记录平均亮度、近黑比例、色相分布、突变像素和局部色斑比例
- `src/concept_palette.py`：从原始概念图按空间提取 1～3 个代表色，合并近似颜色，
  估计面积比例，本地编译光效提示词并生成宽幅渐变；不再反向修改概念图颜色
...
```text
快速模式：中文场景 → 自然概念提示词 → SD 1.5 概念图
                  └→ 独立光效提示词 → 严重偏色检测 → 确定性 Raw 光效图

质量模式：中文场景 → DeepSeek V4 Flash 英文提示词 → SD 1.5 + LoRA → Raw 光效图
```
...
快速共享色板路径会在渲染后执行场景感知的亮度下限。普通场景、活力/霓虹
场景和明确暗场景分别使用不同阈值；系统先进行不裁剪通道的线性增益，仍不足时
再少量向线性白点提升。判断同时考虑 RGB 通道峰值，饱和红、蓝等颜色不会仅因
相对亮度权重较低而被错误混白。处理前后亮度、通道峰值、策略、实际增益和白点混合比例记录在
`prompt_color_guidance.brightness_floor`，该步骤不重新调用扩散模型。
...
最终 Raw 图必须通过平均亮度、近黑像素、突变像素和局部色斑等质量门禁；
绿色、青色及其他正常色相不再触发失败。带有效空间颜色锚点的 Prompt 还必须
通过布局误差门禁。未通过
