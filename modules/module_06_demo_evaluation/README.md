...
当前使用本地 Gradio 界面通过两条独立链路输出自然实体场景概念图和宽幅光色图。
只有双方主色严重冲突时才提取概念图色板纠正光效，并保留 Raw、模块五增强与
SDL 工程结果。
...
- 所有输入都优先由模块一大模型在 3.0 秒上限内同时生成自然概念提示词和独立
  光效提示词；失败后才使用本地应急双提示词
- 绿色、青色及其他完整色相均可生成
- 四步 LCM 实体概念图与独立宽幅渐变；概念图色板仅作为严重偏色兜底
- 普通、活力/霓虹和明确暗场景的分级亮度下限，避免非暗场景出现大片近黑区域
- 记录各阶段耗时和 6 秒目标是否达成
...
Stable Diffusion 和概念图加速适配器会在网页开放前加载，加载时间不计入生成耗时。
...
--generation-mode fast|quality  双图快速模式或原完整质量模式
--time-budget-seconds 6         双图目标耗时
--auto-palette-timeout-seconds 3.0  模块一双提示词等待上限
```
...
快速模式左侧生成实体场景概念图，右侧输出可用于灯具表达的抽象光色图。
