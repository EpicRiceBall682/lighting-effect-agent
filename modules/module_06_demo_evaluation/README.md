...
当前使用本地 Gradio 界面输出一张带实体场景的概念图和一张共享同一色彩蓝图的
宽幅光色图，并保留 Raw、模块五增强与 SDL 工程结果。
...
- 明确颜色走本地快速路径；无明确颜色时模块一大模型在 2 秒上限内自由配色，
  失败后自动使用本地场景与风格语义兜底
- 绿色、青色及其他完整色相均可生成
- 四步 LCM 实体概念图、共享色彩蓝图协调与同蓝图宽幅渐变
...
--auto-palette-timeout-seconds 2  模块一自动配色等待上限
```
